...
@Service
public class SnapshotDefinitionServiceImpl extends BaseServiceImpl<SnapshotDefinition, Integer>
		implements SnapshotDefinitionService, HasLogger {
	...
	public int save(SnapshotDefinition snapshotDefinition, String filename) throws LocalizedException {
		...
		// The Value list is edited and added to the SnapshotDefinition object that
		// should be attached.
		for (SnapshotValue snapshotValue : snapshotDefinition.getSnapshotValues()) {
			if (!snapshotValue.getParentTag().equals(Constants.VALUE_NULL)
					&& snapshotValue.getRowIndex() != Constants.VALUE_CONTROL_ROWID
					&& snapshotValue.getRowIndex() != Constants.VALUE_FILTER_ROWID
					&& !parentIDList.contains(snapshotValue.getParentTag())) {
		...
		// The Snapshot object is edited and added to the SnapshotDefinition object that
		// should be attached.
		...
		// Possible Property objects are retrieved from the document and made ready for
		// save operation.
		// If the SplitterIndex -1 is a different value, the property table
		// splitterIndex is processed in the appropriate format.
		...
		List<Property> propertyListInDB = propertyService.getPropertyListByCriteria(Constants.PROPERTY_SPLITTER,
				String.valueOf(snapshotDefinitionId), true);
	...
	@Override
	public void createLookupParameterContent(SnapshotDefinition snapshotDefinition, TestCase newTestCase)
			throws LocalizedException {
		for (SnapshotValue snapshotValue : snapshotDefinition.getSnapshotValues()) {
			if (StringUtils.isEmpty(snapshotValue.getLookUpParameter())
					|| snapshotValue.getLookUpParameter().equals(Constants.VALUE_NULL)) {
				continue;
			}
	...
	// the generator is created by determining which TestStep belongs to the
	// incoming lookupParameter. The lookupParameter is fragmented and cloned to
	// create the new one.
	...
	private void doFillTestStepOperations(TestCase newTestCase, Generator generator) throws LocalizedException {
		// The clone operation is performed and saved according to the
		// snapshotDefinitionId contained in the lookup.
		...
		findAndUpdateLookUp(String.valueOf(tempSnapshotDefinition.getId()),
				String.valueOf(newSnapshotDefinition.getId()), newTestCase.getLookUps());
	}

	/**
	 * this method findAndUpdateLookUp changes the old snapshotDefinitionId and the
	 * newly created snapshotDefinitionId to change the lookup parameters of
	 * testCase.<br>
	 *
	 ...
	 */
	private void findAndUpdateLookUp(String oldSnapshotDefinitionId, String newSnapshotDefinitionId,
			List<TestStep> lookUps) {
