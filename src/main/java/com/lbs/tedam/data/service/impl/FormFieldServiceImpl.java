...
public class FormFieldServiceImpl extends BaseServiceImpl<FormField, Integer> implements FormFieldService {
     ...
     * If the FormDefinition that the parameters are sent to is in the FormDefinition tab before the snapshotCollector works (existenceCond = true): The difference between
     * the fields in the FormField table and the fields in the snapshot are added. If there is a difference, the xml doc is added to the Snapshot table.<br>
    ...
    private List<FormField> createOrUpdateFormDefinition(Element xmlDoc, FormDefinition formDefinition, boolean existenceCond, String version, NodeList nodes) throws LocalizedException {
        List<FormField> incomingFieldList = new ArrayList<FormField>();
        if (!existenceCond) {
    ...
    /**
     * Converts the values of the Control values in the given NodeList to the FormField objects and inserts
     * them into the formFields list of the given formDefinition.
     * <p>
    ...
    private void addFieldsToDefinition(NodeList nodes, FormDefinition formDefinition, String version) {
        for (int i = 0; i < nodes.getLength(); i++) {
            ...
            formField.setMandatory(nodes.item(i).getAttributes().getNamedItem(Constants.SNAPSHOT_CONTROL_ATTRIBUTES_MANDATORY).getNodeValue().equalsIgnoreCase(Constants.VALUE_TRUE)
                    ? true : false);

            formField.setLookUp(nodes.item(i).getAttributes().getNamedItem(Constants.SNAPSHOT_CONTROL_ATTRIBUTES_HAS_LOOKUP).getNodeValue().equalsIgnoreCase(Constants.VALUE_TRUE)
                    ? true : false);
    ...
    private FormField fillAndGetFormFieldsWithAttributes(FormDefinition formDefinition, NamedNodeMap tempAttributes) {
        ...
        formField.setMandatory(tempAttributes.getNamedItem(Constants.SNAPSHOT_CONTROL_ATTRIBUTES_MANDATORY)
                .getNodeValue().equalsIgnoreCase(Constants.VALUE_TRUE) ? true : false);
        formField.setLookUp(tempAttributes.getNamedItem(Constants.SNAPSHOT_CONTROL_ATTRIBUTES_HAS_LOOKUP).getNodeValue()
                .equalsIgnoreCase(Constants.VALUE_TRUE) ? true : false);
