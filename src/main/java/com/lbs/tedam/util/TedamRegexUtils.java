...
public class TedamRegexUtils {
	...
	/**
	 * Parse the lookupParameter information in the SnapshotValue and return it in
	 * the testStep style.
	 *
		...
		try {
			...
			while (loopCond) {
				if (sbParamWithRegex.toString().startsWith(TestStepType.GRID_SEARCH.getBeginRegex())) {
					// For lookup operations of type GridSearch, the text between the <gs> and </
					// gs> regexes is taken at the parameter in our hand.
					int firstIndex = TestStepType.GRID_SEARCH.getBeginRegex().length();
					int lastIndex = sbParamWithRegex.indexOf(TestStepType.GRID_SEARCH.getEndRegex(),
							TestStepType.GRID_SEARCH.getBeginRegex().length());
					...
					checkForparamListLength(paramList, lookupParam);
					...
					int lastIndex = sbParamWithRegex.indexOf(TestStepType.FILTER_FILL.getEndRegex(),
							TestStepType.FILTER_FILL.getBeginRegex().length());
					String param = sbParamWithRegex.substring(firstIndex, lastIndex);
					...
					int lastIndex = sbParamWithRegex.indexOf(TestStepType.FORM_FILL.getEndRegex(),
							TestStepType.FORM_FILL.getBeginRegex().length());
					String param = sbParamWithRegex.substring(firstIndex, lastIndex);
					...
					int lastIndex = sbParamWithRegex.indexOf(TestStepType.GRID_CELL_SELECT.getEndRegex(),
							TestStepType.GRID_CELL_SELECT.getBeginRegex().length());
					String param = sbParamWithRegex.substring(firstIndex, lastIndex);
					...
					lookupParam.addParameters(ScriptParameters.ROW_INDEX,
							paramList[1].replace("[", "").replace("]", ""));
					...
					int lastIndex = sbParamWithRegex.indexOf(TestStepType.GRID_ROW_SELECT.getEndRegex(),
							TestStepType.GRID_ROW_SELECT.getBeginRegex().length());
					String param = sbParamWithRegex.substring(firstIndex, lastIndex);
					...
					int lastIndex = sbParamWithRegex.indexOf(TestStepType.GRID_DOUBLE_CLICK.getEndRegex(),
							TestStepType.GRID_DOUBLE_CLICK.getBeginRegex().length());
					String param = sbParamWithRegex.substring(firstIndex, lastIndex);
					...
					int lastIndex = sbParamWithRegex.indexOf(TestStepType.ROW_COUNT_VERIFY.getEndRegex(),
							TestStepType.ROW_COUNT_VERIFY.getBeginRegex().length());

					// The buttonClick parameters are parsed based on the
					// !ps! regex.
					String[] param = sbParamWithRegex.substring(firstIndex, lastIndex)
							.split(Regex.PARAMETER_SPLITTER.getRegex());
					...
					int lastIndex = sbParamWithRegex.indexOf(TestStepType.VERIFY.getEndRegex(),
							TestStepType.VERIFY.getBeginRegex().length());
					// The buttonClick parameters are parsed based on the !ps! regex.
					String[] params = sbParamWithRegex.substring(firstIndex, lastIndex)
							.split(Regex.PARAMETER_SPLITTER.getRegex());
					LookupParameter lookupParam = new LookupParameter(VerifyTypes.FIELD_VALUE_VERIFY.getType());
					checkForVerifyParamLength(params, lookupParam);
					...
					int lastIndex = sbParamWithRegex.indexOf(TestStepType.MESSAGE_VERIFY.getEndRegex(),
							TestStepType.MESSAGE_VERIFY.getBeginRegex().length());
					...
					int lastIndex = sbParamWithRegex.indexOf(TestStepType.BUTTON_CLICK.getEndRegex(),
							TestStepType.BUTTON_CLICK.getBeginRegex().length());
					...
					checkForButtonClickParamLength(paramList, lookupParam);
					...
					int lastIndex = sbParamWithRegex.indexOf(TestStepType.POPUP.getEndRegex(),
							TestStepType.POPUP.getBeginRegex().length());
					...
					// Since there are no scenarios where rowindex and column tag are set, the
					// condition for the first 2 cases specified in BaseBshGenerator is written.
					checkForPopupParamLength(paramList, lookupParam);
	...
	private static void checkForPopupParamLength(String[] paramList, LookupParameter lookupParam) {
		if (paramList.length == 1) {
			lookupParam.addParameters(ScriptParameters.POP_UP_TAG, paramList[0]);
		} else if (paramList.length == 2) {
			// The gridTag is not set here because there is no popup in the lookup when I
			// look at the scenario created up to now.
	...
	private static void checkForButtonClickParamLength(String[] paramList, LookupParameter lookupParam) {
		if (paramList.length > 1) {
	...
	private static void checkForVerifyParamLength(String[] params, LookupParameter lookupParam) {
		if (params.length == 1) {// the old data only has uploadedSnapshotId. Since there is no IS_IGNORE_ROW
									// INDEX in the old, all the parameters.length with a value
	...
	private static void checkForparamListLength(String[] paramList, LookupParameter lookupParam) {
		if (paramList.length > 1) {
	...
	public static List<String> decomposeRangeValues(String parameterCollection) {
		...
		if (parameterCollection.contains(Constants.VALUE_REGEX_EXCLUDE)) {
			...
			if (parameterCollection.contains(Constants.VALUE_REGEX_GROUPPED)) {
				parameters.set(0, parameterCollection.substring(sizeOfRegexGroupped, index));
			} else if (parameterCollection.contains(Constants.VALUE_REGEX_LOWVALUE)
					&& parameterCollection.contains(Constants.VALUE_REGEX_HIGHVALUE)) {
